...
def generic_forward(x,w,b,gamma=None,beta=None,bn_param=None,ln_param=None,do_param=None,last=False):
    bn_cache,relu_cache,ln_cache,do_cache = None, None, None, None
    out,fc_cache = affine_forward(x, w, b)

    if not last:
        if bn_param is not None:
          out,bn_cache = batchnorm_forward(out, gamma, beta, bn_param)
        elif ln_param is not None:
          out,ln_cache = layernorm_forward(out,gamma,beta,ln_param=ln_param)
    ...
    cache = (fc_cache,bn_cache,relu_cache,ln_cache,do_cache)
    return out,cache

def generic_backward(dout, cache):
    dgamma, dbeta = None, None
    fc_cache, bn_cache, relu_cache,ln_cache,do_cache = cache
